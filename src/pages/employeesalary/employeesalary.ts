...
import { EmployeesalaryinsertPage } from '../employeesalaryinsert/employeesalaryinsert';
import { PhonebillinsertPage } from '../phonebillinsert/phonebillinsert';
...
export class EmployeesalaryPage {
  ...
  insertPosition(){
    this.navCtrl.push(EmployeesalaryinsertPage);
  }
  showdata() {
    this.selectData();
      ...
      if (this.selectedSearchtype == "ရာထူးအမ်ိဳးအစား") {
        if (this.val && this.val.trim() != '') {
          this.Outcome = this.Outcome.filter((item) => {
            return (item.position.toLowerCase().indexOf(this.val.toLowerCase()) > -1);
          })
        }
        else {
          this.Outcome = [];
        ...
        if (data.rows.length > 0) {
          for (var i = 0; i < data.rows.length; i++) {
            this.listsOutcome.push({
              ...
              position: data.rows.item(i).subtype,
              salary: data.rows.item(i).outcometype,
  ...
  edit(item) {
    ...
    this.navCtrl.push(EmployeesalaryinsertPage, data);
  }
  delete(item) {
