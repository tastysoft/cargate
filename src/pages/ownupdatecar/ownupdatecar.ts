...
import { OwnupdatecarinsertPage } from '../ownupdatecarinsert/ownupdatecarinsert';
import { File } from '@ionic-native/file';

@Component({
  selector: 'page-ownupdatecar',
  templateUrl: 'ownupdatecar.html',
})
export class OwnupdatecarPage {
  num: number;
  dates: string;
  searchData: any;
  val: any;
  ...
  insertPhonebill() {
    this.navCtrl.push(OwnupdatecarinsertPage);
  }
  message(mes) {
  ...
  selectData() {
    this.database.executeSql("Select id,cartype,carnumber,outcome,date,remark from ownUpdatecar ", {})
      .then((data) => {
        this.listsIncomeReg = [];
        if (data.rows.length > 0) {
          for (var i = 0; i < data.rows.length; i++) {
            this.listsIncomeReg.push({
              ...
              outcome: data.rows.item(i).outcome,
  ...
  edit(item) {
    ...
    this.navCtrl.push(OwnupdatecarinsertPage, data);
  }
  delete(item) {
    let confirm = this.alertCtrl.create({
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            let ids = item.id;
            this.database.executeSql("delete from ownUpdatecar where id = " + ids + "", [])
              .then(() => {
