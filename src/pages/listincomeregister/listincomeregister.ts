...
import { IncomeregisterinsertPage } from '../incomeregisterinsert/incomeregisterinsert';
...
@Component({
  selector: 'page-listincomeregister',
  templateUrl: 'listincomeregister.html',
})
export class ListincomeregisterPage {
  num: number;
  dates: string;
  searchData: any;
  val: any;
  ...
  constructor(public navCtrl: NavController, public navParams: NavParams, private sqlite: SQLite, private alertCtrl: AlertController, private datePicker: DatePicker, private keyboard: Keyboard, public file: File) {
  ...
  insertPhonebill(){
    this.navCtrl.push(IncomeregisterinsertPage);
  }
  message(mes) {
  ...
  filterData() {
    if (this.selectedSearchtype == "" || this.selectedSearchtype == undefined) {
      ...
      } else if (this.selectedSearchtype == "ခရီးစဥ္အမ်ိဳးအစား") {
        if (this.val && this.val.trim() != '') {
          this.IncomeReg = this.IncomeReg.filter((item) => {
  ...
  selectData() {
    this.database.executeSql("Select id,track,cartype,carnum,owner,tracktype,date,remark from incomeReg ", {})
      .then((data) => {
        this.listsIncomeReg = [];
  ...
  edit(item) {
    ...
    this.navCtrl.push(IncomeregisterinsertPage, data);
  }
  delete(item) {
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            ...
            this.database.executeSql("delete from incomeReg where id = " + ids + "", [])
              .then(() => {
