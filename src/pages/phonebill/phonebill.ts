...
@Component({
  selector: 'page-phonebill',
  templateUrl: 'phonebill.html',
})
export class PhonebillPage {
  num: number;
  dates: string;
  searchData: any;
  val: any;
  ...
  insertPhonebill(){
    this.navCtrl.push(PhonebillinsertPage);
  }
  showdata() {
    this.selectData();
        ...
        if (data.rows.length > 0) {
          for (var i = 0; i < data.rows.length; i++) {
            this.listsOutcome.push({
              ...
              phonebill: data.rows.item(i).outcometype,
  ...
  edit(item) {
    ...
    this.navCtrl.push(PhonebillinsertPage, data);
  }
  delete(item) {
