...
import { RentinsertPage } from '../rentinsert/rentinsert';
...
export class RentPage {
  ...
  insertRent(){
    this.navCtrl.push(RentinsertPage);
  }
  showdata() {
    this.selectData();
        ...
        if (data.rows.length > 0) {
          for (var i = 0; i < data.rows.length; i++) {
            this.listsOutcome.push({
              ...
              rent: data.rows.item(i).outcometype,
  ...
  edit(item) {
    ...
    this.navCtrl.push(RentinsertPage, data);
  }
  delete(item) {
